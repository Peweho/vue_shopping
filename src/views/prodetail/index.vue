  ...
  <div class="prodetail">
    ...
    <div class="footer">
      ...
      <div @click="$router.push('/cart')" class="icon-cart">
        <div v-if="cartTotal > 0" class="num">{{ cartTotal }}</div>
...
export default {
  ...
  methods: {
    ...
    async getComment () {
      const res = await getProComment(this.goodsId, this.limit)
    ...
    async addCart () {
      ...
      const { data } = await addItemToCart(this.goodsId, this.count, this.detail.skuList[0].goods_sku_id)
